...
ALTER TABLE
    `user` ADD UNIQUE `user_cpf_unique`(`cpf`);
ALTER TABLE
    `user` ADD UNIQUE `user_phone_unique`(`phone`);

ALTER TABLE
    `user` ADD CONSTRAINT `fk_user_type_id` FOREIGN KEY(`user_type_id`) REFERENCES `user_type`(`user_type_id`);
ALTER TABLE
    `user` ADD CONSTRAINT `fk_subscriptions_type_id` FOREIGN KEY(`subscriptions_type_id`) REFERENCES `subscriptions_type`(`subscriptions_type_id`);
...
ALTER TABLE
    `user_payment_info` ADD CONSTRAINT `fk_user_id` FOREIGN KEY(`user_id`) REFERENCES `user`(`user_id`);
